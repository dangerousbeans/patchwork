...
module.exports = function (sbot, config, myId) {
  config = config || {}
  var conf = config.notifier || {}
  var wsConf = config.ws || {}
  var wsHost = wsConf.host || 'localhost'
  var wsPort = wsConf.port || 8989
  var msgBaseUrl = conf.msgBaseUrl || ('http://' + wsHost + ':' + wsPort + '/msg/')

  function makeUrl(msg) {
    return msgBaseUrl + encodeURIComponent(msg.key)
  }
